...
function service(data) {
    ...
    .fcall( () => {
        ...
        return models.apiuser.findOne({raw:true, where: {id: params.apiuser}});
        
    })
    .then(apiuser => {
        if(!apiuser) throw new Error('Invalid user id passed');

        d.resolve(obval.exclude(['passwordhash', 'updatedAt', 'createdAt', 'deletedAt', 'isd', 'api_key']).from(apiuser));
    })
    .catch( e => {
