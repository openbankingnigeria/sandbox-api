...
function service(data) {
    ...
    .fcall( () => {
        ...
        return [models.atm.findAndCountAll({
            limit,
            offset,
            raw: true,
            where: query_where,
    ...
    .spread( (atms, params) => {
        ...
        let response = atms.rows.map(normalize)
        
        if (params.fetch_one) {
            response = response[0];
            if (!response) throw new Error("ATM not found");
        ...
        d.resolve(paginate(response, 'atms', atms.count, limit, page));
    })
    .catch( e => {
