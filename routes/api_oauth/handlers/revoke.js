...
const revokeService = require('mlar').mreq('services', 'auth/oauthmod').revokeToken;
...
function vinfo(req, res, next){ 
        ...
        .then(function(token) {
          ...
          revokeService(token, (err) => {
            // console.log(arguments);
            if(err) {
              utils.jsonF(res, null, err.message);
              return;
            }
            utils.jsonS(res, "Token invalidated", "Process completed successfully")
          });
...
vinfo.routeConfig = [{}, {}];
vinfo.routeConfig[0].path = "/revoke"; 
vinfo.routeConfig[0].method = "get"; 
vinfo.routeConfig[0].middlewares = [routemeta('oauth_authorize', 'oauth_gtoken')];

vinfo.routeConfig[1].path = "/logout"; 
vinfo.routeConfig[1].method = "get"; 
vinfo.routeConfig[1].middlewares = [routemeta('oauth_authorize', 'oauth_gtoken')];
