...
const service = require('mlar').mreq('services', 'billpayment/search_category');
const routemeta = require('mlar')('routemeta');
function vinfo(req, res, next){ 
        const service_data = {...req.body, ...req.query, ...req.headers, ...req.params};
        service(service_data)
        .then( service_response => {
            utils.json(res, service_response, "SUCCESSFUL", "00");
        })
        .catch( service_error => {
...
vinfo.routeConfig = {};
vinfo.routeConfig.path = "/billers/categories"; //no param customer id will be inferred from the header token passed
vinfo.routeConfig.method = "get"; 
vinfo.routeConfig.middlewares = [routemeta('billpayment_categories', 'dev')];
